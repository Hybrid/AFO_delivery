    ...
    def message_track(self, cr, uid, ids, tracked_fields, initial_values, context=None):
        ...
        def format_message(message_description, tracked_values):
            ...
            for name, change in tracked_values.items():
                ...
                col_type = change.get('col_type')
                if col_type == 'one2many':
                    if old_value:
                        old_value = set(old_value) - set(new_value)
                        if len(old_value) > 0:
                            st = ''.join([str(i.id) + ',' for i in old_value])
                            message += 'Some entities were removed! [%s]' % st
                    new_value = set(new_value) - set(change.get('old_value'))
                    if len(new_value) > 0:
                        message += 'The following entities were added: <br/> <table><tr><td>'
                        new_value = '<br/>'.join([i.name for i in new_value])
                        message += '%s' % new_value
                        message += '</td></tr></table>'
                    message += '</div>'
                else:
